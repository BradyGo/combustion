...
// "G:\燃烧优化（邢）\优化程序含优化算法\优化程序\TaddM\src" "-w"
// "enable:specified_file_mismatch" "-w" "enable:repeated_file" "-w"
// "enable:switch_ignored" "-w" "enable:missing_lib_sentinel" "-w"
// "enable:demo_license" "-v"
// "G:\燃烧优化（邢）\优化程序含优化算法\优化程序\Tadd.m" 
//

#include <stdio.h>
#define EXPORTING_TaddM 1
#include "TaddM.h"
    ...
    mclOutputHandlerFcn print_handler)
{
  ...
  if (!mclmcrInitialize())
    return false;
  if (!GetModuleFileName(GetModuleHandle("TaddM"), path_to_dll, _MAX_PATH))
    return false;
    {
...
{
  return TaddMInitializeWithHandlers(mclDefaultErrorHandler, mclDefaultPrintHandler);
}
...
{
  return mclFeval(_mcr_inst, "Tadd", nlhs, plhs, nrhs, prhs);
}

LIB_TaddM_CPP_API 
void MW_CALL_CONV Tadd(int nargout, mwArray& m, const mwArray& a, const mwArray& b)
{
  mclcppMlfFeval(_mcr_inst, "Tadd", nargout, 1, 2, &m, &a, &b);
}

...
t3 = toc
%z figure(1)
%z plot(tY1,'k-','linewidth',1.5) ,hold on ,plot(tYFT11,'k--','markersize',4.5,'linewidth',1.5) ,hold on ,plot(Y11_new,'k-s','markersize',4.5,'linewidth',1.3) ,hold off
%坐标标注
%z xlabel('样本序列','FontSize',10.5,'FontName','宋体');
%z ylabel('排烟温度/℃','FontSize',10.5,'FontName','宋体')
%刻度字体大小
%z set(gca,'FontSize',10.5)
%设置legend
%z h=legend('实际值','模型预测值','模型优化值');
%z set(h,'Fontsize',10.5,'FontName','宋体');
% legend('boxoff')
%z box off%去掉图形中的右框线和上框线
%设置纵坐标
%z  ylim([320,390]);%为纵坐标设置范围
%计算误差
avg1 = mean(tY1)
avg11 = mean(tYFT11)
avg111 = mean(Y11_new)
%z set (gcf,'windowstyle','normal')
%z set (gcf,'Position',[1000,300,1000,300])
% E1 = sum((Y11_new'-tYFT11)./tYFT11)/50       %平均相对误差
%%%%%%%%%%%%%%%%%%%%%%%%%%%%% NOx排放   优化前后对比效果%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%z figure(2)
%z plot(tY2,'k-','linewidth',1.5) ,hold on ,plot(tYFT22,'k--','markersize',4,'linewidth',1.5) ,hold on ,plot(Y22_new,'k-s','markersize',4.5,'linewidth',1.3) ,hold off
%坐标标注
%z xlabel('样本序列','FontSize',10.5,'FontName','宋体'); 
%z ylabel('NO_x排放量/（mg·m^-^3）','FontSize',10.5,'FontName','宋体');
%刻度字体大小
%z set(gca,'FontSize',10.5)
%设置legend
%z h=legend('实际值','模型预测值','模型优化值');
